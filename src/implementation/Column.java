...
abstract public class Column implements Callable<Double> {
	...
	protected Grid grid;
	/**
	 * ueber das grid kommt die column mit grid.getLOcals an die locale
	 * schrittzahl ran.
	 **/
	...
	synchronized protected double getDelta(Hashtable<Integer, Double> leftAccu, Hashtable<Integer, Double> outLeft,
		...
		double delta = 0.0;
		for (int j = 0; j < graph.height; j++) {
			double val = ((leftAccu.getOrDefault(j, 0.0) + rightAccu.getOrDefault(j, 0.0))
					- (outLeft.getOrDefault(j, 0.0) + outRight.getOrDefault(j, 0.0)));
			delta = val * val + delta;
		}
		return delta;
	}
	 ...
	 * 
	 * public double getSum(){ double ret= 0.0; Iterator<Entry<Integer,Double>>
	 * row = values.entrySet().iterator(); while(row.hasNext()) ret = ret +
	 * row.next().getValue(); return ret; }
	 */
